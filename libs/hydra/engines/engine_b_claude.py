...
class EngineB_Claude(BaseEngine):
    ...
    def make_trade_decision(
        ...
        if abs(funding_rate) < 0.005:  # 0.5% threshold
            logger.info(f"[Engine B] Funding not extreme ({funding_rate:.3%} < 0.5%) - HOLD")
            return None

        # Get current stats for prompt injection
        stats = self.portfolio.get_stats()
        tournament_summary = self.tournament_manager.get_tournament_summary()
        my_rank = next((r for r in tournament_summary["rankings"] if r["gladiator"] == "B"), None)
...
- Rank: #{my_rank['rank']}/4
- Weight: {my_rank['weight'] * 100:.0f}% (determines your influence)
...
"""
        ...
        return f"""You are Gladiator B, an INDEPENDENT trader specializing in LOGIC VALIDATION.

TOURNAMENT COMPETITION:
- You are COMPETING against 3 other engines (A, C, D)
- You trade INDEPENDENTLY with your own portfolio
- Your performance determines your ranking and weight
...
Your job: Make INDEPENDENT trading decisions based on LOGICAL ANALYSIS and RED-TEAM THINKING.
...
{{
  ...
  "reasoning": "Logical analysis (mention regime alignment and any concerns)"
    ...
    def _build_validation_system_prompt(self) -> str:
        """Build system prompt for strategy validation (legacy method)."""
