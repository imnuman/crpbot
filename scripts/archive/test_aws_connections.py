    ...
    for name, arn in secrets:
        try:
            ...
            if name == 'Coinbase API':
                ...
                print(f"  ✅ {name}: {'Real credentials' if has_real_data else 'Placeholder values'}")
                if has_real_data:
                    print(f"    - API Key: {secret_data.get('api_key', '')[:20]}...")
                    print(f"    - Private Key: {'Present' if secret_data.get('private_key') else 'Missing'}")
...
def test_coinbase_api():
    ...
    try:
        ...
        if api_key.startswith('organizations/') and 'BEGIN EC PRIVATE KEY' in private_key:
            ...
            print(f"    - Private Key: PEM format detected")
        else:
            print("  ❌ Coinbase credentials format incorrect")
            print(f"    - API Key should start with 'organizations/'")
            print(f"    - Private Key should be PEM format")
